...
public class Sportsman {
	...
	public static void findBestSportsman(Sportsman[] sportsmen) {
		...
		int maxPoints = 0;
		if (bestPlayer instanceof FootballPlayer) {
			bestPlayer = new FootballPlayer();
			for (int i = 0; i < sportsmen.length; i++) {
				FootballPlayer currentPlayer = (FootballPlayer) sportsmen[i];
				int currentPlayerPoints = currentPlayer.getCountOfGoals();
		...
		else if (bestPlayer instanceof BasketballPlayer) {
			bestPlayer = new BasketballPlayer();
			for (int i = 0; i < sportsmen.length; i++) {
				BasketballPlayer currentPlayer = (BasketballPlayer) sportsmen[i];
				int currentPlayerPoints = currentPlayer.getSeasonPoints();
				if (currentPlayerPoints > maxPoints) {
					maxPoints = currentPlayerPoints;
					bestPlayer = currentPlayer;
				}
